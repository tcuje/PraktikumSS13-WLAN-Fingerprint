...
public class IPMapView extends View {
                 ...
                 for(int i = 0; i < aPathRout.length; i++){
					...
					case 'M':
						...
						if(aPathRout[i].length()==0){
							i++; //f�r den fall das ein lerzeichen zwichen Befehl und koordinate steht
						}
					...
					case 'm':
						...
						aCoordinate = aPathRout[i].split(",");
						aPath.moveTo((Float.parseFloat(aCoordinate[0])*mWidth), (Float.parseFloat(aCoordinate[1])*mHeight));
						break;
					case 'L':
						aPathRout[i] = aPathRout[i].substring(1);
						if(aPathRout[i].length()==0){
					...
					case 'l':
						...
						aCoordinate = aPathRout[i].split(",");
						aPath.lineTo((Float.parseFloat(aCoordinate[0])*mWidth), (Float.parseFloat(aCoordinate[1])*mHeight));
						break;
					case 'C':
						aPathRout[i] = aPathRout[i].substring(1);
						if(aPathRout[i].length()==0){
					...
					case 'c':
						...
						aPath.cubicTo(Float.parseFloat(aCoordinate[4]), Float.parseFloat(aCoordinate[5]), 
								Float.parseFloat(aCoordinate[0]), Float.parseFloat(aCoordinate[1]), 
								(Float.parseFloat(aCoordinate[2])*mWidth), (Float.parseFloat(aCoordinate[3])*mHeight));
						break;
					case 'z':
						aPath.close();
						break;
                	...
                	if(xpp.getAttributeValue(null, "fill-opacity")!=null){
                		if(xpp.getAttributeValue(null, "fill-opacity").equals("0")){
                			myPaths.add(aPath);
                		}else{
	...
	protected void setMeasurePoint(float x, float y) {
		mXMPoint = (x/mScaleFactor)-(mViewWidth/(2*mScaleFactor)) + mAccXPoint;
		mYMPoint = (y/mScaleFactor)-(mViewHeight/(2*mScaleFactor)) + mAccYPoint;
	...
	private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
	    ...
	    public boolean onScale(ScaleGestureDetector detector) {
	        ...
	        mScaleFactor = Math.max(mMinScaleFactor, Math.min(mScaleFactor, 5.0f));
