...
public class WPSQuadKnot {
	...
	public WPSQuadKnot(float xMi, float xMa, float yMi, float yMa,
		...
		bl = null;
		br = null;
		// System.out.println("neuer Knoten: "+ xMin+"  " + xMax+"  " +
		// yMin+"  " + yMax);
		mapView = mView;
	}
		...
		if (tl == null && value == null) {
			...
			if (tl == null) {
				...
				bl = new WPSQuadKnot(xMin, (xMax + xMin) / 2,
						(yMin + yMax) / 2, yMax, mapView);
				br = new WPSQuadKnot((xMax + xMin) / 2, xMax,
						(yMin + yMax) / 2, yMax, mapView);
				mapView.drawLine(new PointF((xMax + xMin) / 2, yMin),
						new PointF((xMax + xMin) / 2, yMax));
				mapView.drawLine(new PointF(xMin, (yMin + yMax) / 2),
						new PointF(xMax, (yMin + yMax) / 2));
			}
			if (value != null) {
